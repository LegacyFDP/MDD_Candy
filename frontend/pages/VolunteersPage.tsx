...
import { Plus, Pencil, Trash2, Users, CalendarRange, Printer, Tent } from 'lucide-react'
...
export default function VolunteersPage({ currentUser }: Props) {
  ...
  const volunteersByEvent = useMemo(() => {
    const grouped = new Map<number, FeteVolunteerGroup>()
    for (const shift of shifts) {
      ...
      if (!grouped.has(shift.fete_id)) {
        grouped.set(shift.fete_id, {
          ...
          eventDate: fetes.find(fete => fete.id === shift.fete_id)?.event_date ?? '',
          shifts: [],
        })
      }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" asChild className="flex items-center gap-2">
            <Link to="/print-lists"><Printer className="w-4 h-4" /> Print Centre</Link>
          </Button>
      ...
      <div className="border rounded-lg bg-card p-4 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <Select value={shiftVolunteerFilter} onValueChange={setShiftVolunteerFilter}>
              <SelectTrigger className="w-48"><SelectValue placeholder="Filter by volunteer" /></SelectTrigger>
              <SelectContent>
        ...
        {filteredShifts.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            {shiftVolunteerFilter === 'all' ? 'No volunteer shifts yet.' : 'No shifts for this volunteer.'}
          </p>
        ) : (
      ...
      <div className="border rounded-lg bg-card p-4 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold">Volunteers by event</h2>
        </div>
        ...
        {volunteersByEvent.length === 0 ? (
          ...
          <div className="space-y-3">
            {volunteersByEvent.map(group => (
              <div key={group.feteId} className="border rounded-md p-3">
                <p className="font-medium text-sm">{group.feteName}</p>
                {group.eventDate && (
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {group.shifts.map(shift => (
                    <span key={shift.id} className="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-1 text-xs text-primary">
                      {shift.volunteer_name} ({shift.role}) · {shift.start_date}{shift.start_date !== shift.end_date ? `–${shift.end_date}` : ''} {shift.start_time}–{shift.end_time}
                    </span>
                  ))}
                </div>
              </div>
      ...
      <Dialog open={shiftOpen} onOpenChange={setShiftOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            {shiftError && <p className="text-destructive text-sm">{shiftError}</p>}
          </div>
          <DialogFooter>
